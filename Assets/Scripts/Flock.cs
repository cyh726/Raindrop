...
public class Flock : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            transform.Translate(0, -t* Random.Range(0.001f, 0.0015f), 0);
        }
        ...
        if (transform.position.y < -3)
        {
            ...
            if (transform.localScale.x > 0.1f)
            {
                ...
                transform.localScale = new Vector3(0.12f+ Random.Range(-0.01f, 0.01f), 0.15f+ Random.Range(-0.02f, 0.01f), 0.1f);
            }
            else
            {
                Vector3 small = new Vector3(0.06f + Random.Range(-0.01f, 0.01f), 0.06f + Random.Range(-0.01f, 0.01f), 0.1f);
                    ...
                    if (nDistance < 0.15f*(1+transform.localScale.x))
                    {
                        if (transform.localScale.x > go.transform.localScale.x)
                        {
                            if (go.transform.localScale.x > 0.1f)
                            {
                                ...
                                go.transform.localScale = new Vector3(0.12f + Random.Range(-0.01f, 0.01f), 0.15f + Random.Range(-0.02f, 0.01f), 0.1f);
                                transform.localScale += new Vector3(0.0001f, 0.003f, 0);
                            }
                            else
                            {
                                go.transform.position = transform.position+ new Vector3(0, 0.5f, 0);
                                go.transform.localScale = small;
                                transform.localScale += new Vector3(0.001f, 0.003f, 0);
                            }
